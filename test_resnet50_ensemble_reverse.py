...
def eval_model_variant(model, model2, dataloaders, upc_to_brand):
    ...
    for inputs, labels, paths in dataloaders[phase]:
        ...
        brands_probs, outputs_brands = torch.max(outputs_brands, 1)
        outputs_variants = F.softmax(outputs_variants, dim=1)
        variants_probs, outputs_variants = torch.max(outputs_variants, 1)

        brand_label = list(convert_to_numpy(outputs_brands))
        variants_label = list(convert_to_numpy(outputs_variants))

        changed_upcs = []
        for idx, ot in enumerate(brand_label):
#            print(variants_probs[idx])
            if brands_probs[idx] > 0.7:
#            if variants_probs[idx]  < 0.991:
#            if brands_probs[idx] > 0.95:
               brand = upc_to_brand[ot]
            #out = outputs_variants[idx][upcs]
            #changed_upc = upcs[torch.argmax(F.softmax(out))]
               print("brand:", brand)
               changed_upcs.append(brand)
            else:
                print("variant:", variants_label[idx])
...
if __name__=="__main__":
    ...
    upc_to_brand = pickle.load(open('upc_to_brand.pkl','rb'))
    ...
    since = time()
    accuracy, labels, outputs = eval_model_variant(brand_model, brand_variant_model,  dataloaders, upc_to_brand)
